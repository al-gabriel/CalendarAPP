...
Displays year-specific timeline information.
Occupies the bottom-left cell when in year view mode.
"""
...
class YearInfoModule(tk.Frame):
    """
    Year information module for displaying year-specific timeline data.
    """
    
    def __init__(self, parent: tk.Widget, timeline: Optional[DateTimeline] = None):
        """
        Initialize the Year info module.
    ...
    def setup_ui(self):
        ...
        self.uk_no_visa_label = tk.Label(self.uk_days_frame, text="", 
                                        font=("Arial", 9), fg="#cc7a7a", bg=counts_frame.cget("bg"))
    ...
    def refresh_info(self):
        ...
        try:
            # Update year label with expected days
            is_leap = calendar.isleap(self.current_date.year)
            expected_days = 366 if is_leap else 365
            year_text = f"Year {self.current_date.year} - {expected_days} days"
            ...
            # Display combined UK Residence + No Visa Coverage (same as month module)
            if no_visa_count > 0:
